...
class FactorySegmentation():
    ...
    def __init__(self, args, base_dir=Path.db_root_dir('factory'),split='train'):
        ...
        self.img_path, self.label_path = shuffle(self.img_path, self.label_path, random_state=0)
        
        train_len = int(len(self.img_path) * 0.8)
        if split=="train":
            self.img_path, self.label_path = self.img_path[:train_len], self.label_path[:train_len]
        else:
            self.img_path, self.label_path = self.img_path[train_len:], self.label_path[train_len:]
        self.split = split
